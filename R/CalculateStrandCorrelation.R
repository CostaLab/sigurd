...
CalculateStrandCorrelation <- function(SE, chromosome_prefix = "chrM"){
  ...
  reads_A_fw  <- assays(SE)[["A_counts_fw"]]
  reads_A_rev <- assays(SE)[["A_counts_rev"]]
  ...
  dt <- merge(data.table(summary(reads_A_fw)), 
              data.table(summary(reads_A_rev)), 
  ...
  reads_C_fw  <- assays(SE)[["C_counts_fw"]]
  reads_C_rev <- assays(SE)[["C_counts_rev"]]
  ...
  dt <- merge(data.table(summary(reads_C_fw)),
              data.table(summary(reads_C_rev)),
              by.x = c("i", "j"), by.y = c("i", "j"),
              all = TRUE)[x.x >0 | x.y >0]
  ...
  reads_G_fw  <- assays(SE)[["G_counts_fw"]]
  reads_G_rev <- assays(SE)[["G_counts_rev"]]
  ...
  dt <- merge(data.table(summary(reads_G_fw)),
              data.table(summary(reads_G_rev)),
              by.x = c("i", "j"), by.y = c("i", "j"),
              all = TRUE)[x.x >0 | x.y >0]
  ...
  reads_T_fw  <- assays(SE)[["T_counts_fw"]]
  reads_T_rev <- assays(SE)[["T_counts_rev"]]
  ...
  dt <- merge(data.table(summary(reads_T_fw)),
              data.table(summary(reads_T_rev)),
              by.x = c("i", "j"), by.y = c("i", "j"),
              all = TRUE)[x.x >0 | x.y >0]
