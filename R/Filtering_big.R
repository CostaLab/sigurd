...
Filtering_big <- function(se, blacklisted_barcodes_path = NULL, fraction_threshold = NULL, path_seurat = NULL, min_cells_per_variant = 2){
  ...
  # Fix this part by directly addressing the elements of the sparse matrix.
  if(!is.null(fraction_threshold)){
    ...
    #fraction_matrix  <- as(assays(se)$fraction, "dgTMatrix")
    # i = rows, j = cols, x = values
    #position_matrix_fraction <- rbind(i = fraction_matrix@i, j = fraction_matrix@j, x = fraction_matrix@x)
