...
CallSupport <- function(SE, VOI_group1, VOI_group2, group1_name = "group1", group2_name = "group2", min_mutated_reads = 3, min_reads = 30, group_factor = NULL, verbose = TRUE, return_nonsupport = FALSE){
  ...
  if(!is.numeric(group_factor) & length(group_factor) != 1){
    stop("Your group factor is not a single number.")
  }
  ...
  cells_df$Support <- apply(cells_df[,2:3], 1, function(x){
    if(sum(x) < min_reads){
      return("NoCoverage")
    } else if(x[1] > min_mutated_reads & x[1] > (group_factor * x[2])){
      return(group1_name)
    } else if(x[2] > min_mutated_reads & x[2] > (group_factor * x[1])){
      return(group2_name)
    } else{
      return("NoSupport")
    }
