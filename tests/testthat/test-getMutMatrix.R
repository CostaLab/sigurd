test_that("Testing if getMutMatrix.R works.", {
  ...
  se <- SummarizedExperiment::SummarizedExperiment(assays = list(A_counts_fw = A_counts_fw, A_counts_rev = A_counts_rev,
                                                   ...
                                                   rowRanges = rowRanges)
  # The expected result.
  expected_result_A <-  Matrix::sparseMatrix(i = 1, j = 4, x = 1, dims = c(3,4), dimnames = list(c("chrM_1_G_A", "chrM_3_T_A", "chrM_4_C_A"), paste0("Cell_", 1:4)))
  expected_result_C <-  Matrix::sparseMatrix(i = integer(0), j = integer(0), x = double(0), dims = c(3,4), dimnames = list(c("chrM_1_G_C", "chrM_2_A_C", "chrM_3_T_C"), paste0("Cell_", 1:4)))
  expected_result_G <-  Matrix::sparseMatrix(i = 1, j = 2, x = 1, dims = c(3,4), dimnames = list(c("chrM_2_A_G", "chrM_3_T_G", "chrM_4_C_G"), paste0("Cell_", 1:4)))
  expected_result_T <-  Matrix::sparseMatrix(i = c(1,3,3), j = c(2,3,4), x = c(2/3,1,1), dims = c(3,4), dimnames = list(c("chrM_1_G_T", "chrM_2_A_T", "chrM_4_C_T"), paste0("Cell_", 1:4)))
  expect_equal(sigurd::getMutMatrix(SE = se, cov = coverage + 0.000001, letter = "A", ref_allele = ref_allele, chromosome_prefix = "chrM"),  expected_result_A, tolerance = 1e-6)
  expect_equal(sigurd::getMutMatrix(SE = se, cov = coverage + 0.000001, letter = "C", ref_allele = ref_allele, chromosome_prefix = "chrM"),  expected_result_C, tolerance = 1e-6)
  expect_equal(sigurd::getMutMatrix(SE = se, cov = coverage + 0.000001, letter = "G", ref_allele = ref_allele, chromosome_prefix = "chrM"),  expected_result_G, tolerance = 1e-6)
  expect_equal(sigurd::getMutMatrix(SE = se, cov = coverage + 0.000001, letter = "T", ref_allele = ref_allele, chromosome_prefix = "chrM"),  expected_result_T, tolerance = 1e-6)
})
