...
```{r fig.width = 6, warning = FALSE, dev = "pdf"}
...
```{r Diversity, warning = FALSE, dev = "pdf"}
...
patients <- design_matrix$patient[5]
...
# genotyping_mt_all_ori <- genotyping_mt_all
for(i in 1:length(genotyping_mt_all)) genotyping_mt_all[[i]] <- CombineSEobjects(Filtering(genotyping, cells_include = grep(patients[i], colnames(genotyping), value = TRUE), verbose = FALSE), genotyping_mt_all[[i]])
```
